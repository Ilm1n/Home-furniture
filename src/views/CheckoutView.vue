    ...
    <div v-else-if="currentStep === 2" class="checkout-form">
      ...
      <div class="delivery-options">
        <label
          ...
          :class="[
            'delivery-option',
            { active: formData.deliveryType === option.value },
          ]"
        >
          <input
            type="radio"
      ...
      <div class="form-actions">
        <button type="button" class="back-btn" @click="prevStep">Назад</button>
        <button type="button" class="next-btn" @click="nextStep">
          Продолжить
        </button>
      </div>
    ...
    <div v-else-if="currentStep === 3" class="checkout-form">
      ...
      <div class="payment-options">
        <label
          ...
          :class="[
            'payment-option',
            { active: formData.paymentMethod === option.value },
          ]"
        >
          <input
            type="radio"
...
const deliveryPrice = computed(() => {
  const option = deliveryOptions.value.find(
    (opt) => opt.value === formData.value.deliveryType
  );
...
function validateStep(step) {
  switch (step) {
    case 1:
      return (
        formData.value.firstName && formData.value.email && formData.value.phone
      );
    case 2:
      if (formData.value.deliveryType === "courier") {
        return (
          formData.value.city && formData.value.street && formData.value.house
        );
      }
      return true;
    case 3:
      if (formData.value.paymentMethod === "card") {
        return (
          cardData.value.number && cardData.value.expiry && cardData.value.cvv
        );
      }
      return true;
    default:
